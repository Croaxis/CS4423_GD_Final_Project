...
public class MovePositionPathfinding : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        moveVelocityComponent = GetComponent<IMoveVelocity>();
    }
    ...
    void Update()
    {
        ...
        {
            Vector2 nextWaypoint = pathLeftToGo[0];
            Vector2 moveDirection = (nextWaypoint - (Vector2)transform.position).normalized;
            moveVelocityComponent.SetVelocity(moveDirection);

            float reachedPathPositionDistance = 0.1f;
            if (Vector2.Distance(transform.position, nextWaypoint) < reachedPathPositionDistance){
                transform.position = nextWaypoint;
                pathLeftToGo.RemoveAt(0);
            }
        }
        else{
            moveVelocityComponent.SetVelocity(Vector2.zero);
        }
